...
#include "../../include/qrcode/validation.h"
...
int qrcode_capacity_info(int version, int ecl, int mode, struct qrcode_capacity_info* out_info){
    if (!QRCODE_VALID_VERSION(version)) return QRCODE_ERROR(QRCODE_INVALID_VERSION, -1);
    if (!QRCODE_VALID_ECL(ecl)) return QRCODE_ERROR(QRCODE_INVALID_ECL, -1);

    size_t offset = 16;
    if (QRCODE_IS_MICRO(version)){
        version = QRCODE_VERSION(version);
        if (version == 1){
            if (ecl != QRCODE_ECL_L) return QRCODE_ERROR(QRCODE_INVALID_COMBO | QRCODE_INVALID_VERSION | QRCODE_INVALID_ECL, -1);
            if (mode > QRCODE_MODE_NUMERIC) return QRCODE_ERROR(QRCODE_INVALID_COMBO | QRCODE_INVALID_VERSION | QRCODE_INVALID_ENCODING, -1);
        }
        if (version == 2){
            if (ecl > QRCODE_ECL_M) return QRCODE_ERROR(QRCODE_INVALID_COMBO | QRCODE_INVALID_VERSION | QRCODE_INVALID_ECL, -1);
            if (mode > QRCODE_MODE_ALPHANUMERIC) return QRCODE_ERROR(QRCODE_INVALID_COMBO | QRCODE_INVALID_VERSION | QRCODE_INVALID_ENCODING, -1);
        }
        if (version == 3 && ecl > QRCODE_ECL_M) return QRCODE_ERROR(QRCODE_INVALID_COMBO | QRCODE_INVALID_VERSION | QRCODE_INVALID_ECL, -1);
        if (ecl > QRCODE_ECL_Q) return QRCODE_ERROR(QRCODE_INVALID_COMBO | QRCODE_INVALID_VERSION | QRCODE_INVALID_ECL, -1);
        offset = 0;
    }
    
    *out_info = qrcode_capacity_info_table[offset + (version - 1)*4 + ecl];
    return 0;
}